...
export const api = {
  ...
  casinoStats: () => req('/api/casino/stats'),
  casinoRecentWins: () => req('/api/casino/recent-wins'),
  casinoHistory: () => req('/api/casino/history', { headers: headers() }),
  casinoBiggestLosses: () => req('/api/casino/biggest-losses'),
  ...
  billiardsResult:     (id, b)  => req(`/api/billiards/rooms/${id}/result`, { method:'POST', headers:headers(), body:JSON.stringify(b) }),
  billiardsBetOptions: ()       => req('/api/billiards/bet-options', { headers: headers() }),
  ...
  resetUserPassword: (id, new_password) => req(`/api/admin/users/${id}/reset-password`, { method: 'POST', headers: headers(), body: JSON.stringify({ new_password }) }),
